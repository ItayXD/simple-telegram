...
function SimpleTelegram() {
  ...
  // Listen to conatct loaded and send an event
  var contacts_loaded = function () {
    telegramProcess.stdin.write('contact_list\n', encoding='utf8')
    var contact_list = false;
    var check = function (line) {
      //check if contact_list request was printed already
      if (line.toString().indexOf("contact_list") != -1) { contact_list = true; return}
      //waits for the contacts list
      if ((line.toString().length > 6) && contact_list) {
          //sends the event
          telegramProcess.stdout.emit('contacts_loaded', 'All contacts loaded')
          //stop listening to the event
          telegramProcess.stdout.removeListener("data",check)
      }
    }
    telegramProcess.stdout.on('data', check)
  }
  // Run Telegram process
  var runTelegram = function() {

    var options = ['-WCk', tgKeysFile].concat(extraOptions)
    telegramProcess = require('child_process').execFile(tgBinFile, options)
    contacts_loaded() //Sends contacts_loaded when conacs are loaded == ready
